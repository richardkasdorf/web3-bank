...
if not all([SEPOLIA_RPC, BANK_PRIVATE_KEY, USDC_CONTRACT]):
    raise Exception("❌ Faltam variáveis obrigatórias no arquivo .env (SEPOLIA_RPC, BANK_PRIVATE_KEY ou USDC_CONTRACT)")
...
print(f"✅ Blockchain service carregado - Bank Hot Wallet: {BANK_ADDRESS}")
...
def get_usdc_balance(address: str) -> Decimal:
    ...
    try:
        ...
        raise Exception(f"Erro ao consultar saldo USDC: {str(e)}")
...
def transfer_usdc(to_address: str, amount: Decimal) -> str:
    ...
    try:
        ...
        print(f"✅ Transação enviada: {tx_hash_hex}")
        return tx_hash_hex
